					...
					if($this.find('dl').length > 0) {
						var $dd = $this.find('dd').each(function() {
							$(this).on('click', function() {
								...
								var href = $a.data('url');
								var id = $a.children('i:first').data('id');
				...
				for(var j = 0; j < data[i].children.length; j++) {
					...
					if(data[i].children[j].icon !== undefined && data[i].children[j].icon !== '') {
						if(data[i].children[j].icon.indexOf('fa-') !== -1) {
							ulHtml += '<i class="fa ' + data[i].children[j].icon + '" data-icon="' + data[i].children[j].icon + '" aria-hidden="true" data-id="' + data[i].children[j].id + '"></i>';
						} else {
							ulHtml += '<i class="layui-icon"  data-id="' + data[i].children[j].id + '" data-icon="' + data[i].children[j].icon + '">' + data[i].children[j].icon + '</i>';
						}
					}
